...
function fixture(mutates) {
  ...
  const code=mutates?"require('node:fs').writeFileSync('src/input.txt','changed');console.log('STEP_DONE')":"console.log('STEP_DONE')";
  const p={schemaVersion:1,repository:'fixture/game',authorizedHosts:[os.hostname()],inputs:['src'],steps:{test:{command:['node','-e',code],timeoutSeconds:10,markers:['STEP_DONE']}},gates:{verified:{steps:['test']}}};
  ...
  writeJSON(path.join(root,'.gameprod/lifecycle.json'),Array.from({length:25},(_,i)=>({stage:i})));
  writeJSON(path.join(root,'.gameprod/source.json'),{repository:p.repository,commit:'a'.repeat(40)});
  return {root,p,close:()=>fs.rmSync(root,{recursive:true,force:true})};
}

for(const mutates of [true,false]) {
  test(mutates?'zero-exit source-mutating step is FAILED, not passed':'unchanged source yields a valid passing gate',()=>{
    const f=fixture(mutates);
    try {
      const r=spawnSync(process.execPath,[cli,'run','test'],{cwd:f.root,encoding:'utf8',timeout:20000,env:{...process.env,GITHUB_ACTIONS:'false',GIT_DIR:'',GIT_WORK_TREE:''}});
