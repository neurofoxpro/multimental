...
// Reused from PR91/089f121; live Issue definitions are data, not execution authority.
...
// Every mutation is delegated to the project's existing compare-and-swap edit helper.
export async function synchronizePlan(seed, snapshot, ports) {
  const proposal = proposedSnapshot(seed, snapshot);
  await ports.validateReferences(proposal.plan);
  if (proposal.changes.length) await ports.apply(proposal.plan);
  // Always render: recover safely when the previous write succeeded but rendering did not.
  await ports.render();
  const receipt = {
    schemaVersion: 1,
    repository: REPO,
    ...
    tasks: proposal.plan.tasks.length,
