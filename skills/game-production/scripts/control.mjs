...
export function validatePlan(plan) {
  ...
  for (const task of plan.tasks) {
    ...
    if (typeof task.title !== 'string' || !task.title || typeof task.source !== 'string' || !task.source || typeof task.stage !== 'string' || !task.stage || !STATES.includes(task.status)) {
    ...
    if (task.acceptance.some((x) => typeof x !== 'string' || !x.trim())) throw Error('Empty acceptance text');
    if (!Array.isArray(task.evidence) || !Array.isArray(task.readset)) {
      throw Error('Evidence/readset required: ' + task.id);
    }
    if ([...task.evidence, ...task.readset].some((ref) => !safeRelative(ref))) throw Error('Unsafe input reference: ' + task.id);
...
export function releaseBlockers(plan, ideas = []) {
  ...
  for (const idea of ideas) {
    if (!ideaIsClosed(idea, plan)) blockers.push({ id: idea.id, reason: 'Idea has no completed task or explicit disposition' });
  }
  // This inventory cannot authorize publication, even with all task statuses changed.
  for (const key of ['freshRcQualification', 'ownerReleaseApproval', 'playReadback', 'storeInstall']) {
    blockers.push({ id: key, reason: 'Must be checked by the protected release adapter, not a metadata boolean' });
  }
  return blockers;
}

export function safeRelative(ref) {
  return typeof ref === 'string' && /^[a-zA-Z0-9_.\/-]+$/.test(ref) &&
    !ref.startsWith('/') && !ref.split('/').some((part) => !part || part === '.' || part === '..');
}
...
export function ideaIsClosed(idea, plan) {
  ...
  return ['rejected', 'deferred'].includes(idea.status) && !!idea.reason &&
    safeRelative(idea.decisionEvidence) && Array.isArray(idea.source) && idea.source.length > 0;
}

export function projectBacklog(plan) {
  return plan.tasks.map((task) => ({
    id: task.id, status: task.status === 'verified' ? 'done' : 'pending',
    action: task.title, dependsOn: task.dependsOn, manual: HUMAN_KINDS.includes(task.kind),
    generatedFrom: PLAN_FILE, evidence: task.evidence,
    nextCommand: 'npm run game -- task ' + task.id
  }));
}
...
export function assessReceipt(receipt, expected, readBytes) {
  ...
  if (receipt.sourceChanged === true || (receipt.sourceDigestAfter && receipt.sourceDigestAfter !== receipt.sourceDigest)) return 'source_changed_during_step';
  if (receipt.artifacts !== undefined && !Array.isArray(receipt.artifacts)) return 'invalid_artifacts';
  try {
    for (const item of [...receipt.logs, ...(receipt.artifacts || [])]) {
      if (!safeRelative(item.path) || !HASH.test(item.sha256 || '') || digest(readBytes(item.path)) !== item.sha256) {
...
export function taskPacket(plan, id) {
  ...
  return {
    ...
    task,
    readset: [...new Set(['AGENTS.md', 'NEXT_CHAT.ru.md', 'docs/production/AUTHORITY.md', PLAN_FILE, ...task.readset])],
    sequence: ['read inputs', 'reproduce baseline', 'implement small change', 'check', 'review', 'ship', 'record evidence'],
    commands: {
...
export function renderRoadmap(plan) {
  ...
  return '# Multimental — от идеи до Google Play\n\n' +
    'Источник: `' + PLAN_FILE + '`. Обновлено: ' + plan.updatedAt + '.\n\n' +
    ...
    stages.map((stage) => '## ' + stage + '\n\n' + plan.tasks
      ...
      .map((task) => '### ' + task.id + ' — ' + task.title + '\n\n' +
        'Статус: **' + task.status + '**. Ответственность: ' + task.kind + '. До Play: ' +
        (task.requiredForPlay ? 'да' : 'нет — ' + task.disposition) + '.\n\n' +
        'Зависимости: ' + (task.dependsOn.join(', ') || 'нет') + '.\n\n' +
        task.acceptance.map((line) => '- ' + line).join('\n') + '\n\n' +
        'Пакет работ: `npm run game -- task ' + task.id + '`.\n')
      .join('\n')).join('\n') +
    '\n## Сохранённая история\n\n' +
...
export function commandFor(name, args, plan) {
  if (name === 'start') {
    ...
    if (HUMAN_KINDS.includes(packet.task.kind)) throw Error('Owner/external task is not a code branch');
  ...
  if (name === 'experiment') {
    ...
    return [process.execPath, 'tools/balance.mjs', '--count', '40', '--seed', '9001',
      '--profile', 'current', '--name', args[0] + '-' + crypto.randomUUID()];
  }
  if (name === 'ship') {
    if (args.length !== 1 || !ID.test(args[0])) throw Error('ship requires one task ID');
    const packet = taskPacket(plan, args[0]);
    if (HUMAN_KINDS.includes(packet.task.kind)) throw Error('Human/external gate cannot be shipped as code');
    const blocked = nextTasks(plan).find((task) => task.id === args[0])?.blockedBy || [];
    if (blocked.length) throw Error('Unfinished prerequisites: ' + blocked.join(', '));
    return [process.execPath, OPS, 'cycle', 'feat: ' + args[0], packet.task.title];
  }
  const legacy = new Set([
    'resume', 'begin', 'apply', 'verify', 'audit', 'prepare-sources', 'prepare', 'stage',
    'publish', 'wait', 'integrate', 'wait-dev', 'cycle', 'logs', 'device-status', 'network',
    'qualify', 'bluetooth-pairing', 'bluetooth-room', 'device-pvp', 'device-suite', 'device-test',
    'delivery', 'emulator', 'candidate', 'report', 'resume-cycle', 'record', 'deploy-agent',
    'format', 'changelog', 'handoff', 'readiness', 'research', 'probe-bluetooth'
  ]);
...
export function playPreflight(config, inspection = null) {
  ...
  if (!Number.isInteger(config.minTargetSdk) || config.minTargetSdk < 36) throw Error('Policy target floor missing/obsolete');
  if (config.accountStatus !== 'verified') blockers.push('PLAY_ACCOUNT_UNKNOWN');
  if (config.budgetApproval !== 'existing_account_or_owner_approved') blockers.push('PLAY_BUDGET_UNAPPROVED');
  ...
  if (!inspection) blockers.push('AAB_INSPECTION_MISSING');
    ...
    if (inspection.package !== config.package || inspection.debuggable !== false) blockers.push('INVALID_PRODUCTION_IDENTITY');
    if (!Number.isInteger(inspection.targetSdk) || inspection.targetSdk < config.minTargetSdk) blockers.push('TARGET_API_TOO_LOW');
    ...
    if (!SHA.test(inspection.commit || '') || !HASH.test(inspection.aabSha256 || '')) blockers.push('ARTIFACT_IDENTITY_MISSING');
    if (!Number.isInteger(inspection.versionCode) || inspection.versionCode <= 0) blockers.push('INVALID_VERSION_CODE');
    if (!HASH.test(inspection.uploadCertificateSha256 || '')) blockers.push('UPLOAD_CERTIFICATE_MISSING');
  }
  return {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (process.platform === 'win32') {
    ...
    if (fs.existsSync(path.join(bin, 'git.exe'))) process.env.PATH = bin + path.delimiter + process.env.PATH;
  }
  const withWriteLock = (action) => {
    context(root, readJSON(inside(root, '.gameprod/project.json')));
    const release = acquireOperation(inside(root, '.gameprod/evidence/ops.lock'), { command: 'control-metadata', repository: CANONICAL_REPOSITORY });
    try { return action(); } finally { release(); }
  };
  const plan = validatePlan(readJSON(inside(root, PLAN_FILE)));
  const [name = 'next', ...args] = argv;
  if (['help', 'validate', 'next', 'roadmap', 'closeout'].includes(name) && args.length) throw Error('Unexpected arguments');
  if (name === 'help') {
    console.log('next | task ID | start ID | idea ID TEXT | experiment ID | validate | render [--check] | check | ship ID | resume-cycle | play-check | closeout | legacy ops commands');
    return;
  }
  if (name === 'validate') {
    for (const task of plan.tasks) {
    ...
    if (fs.existsSync(ideas)) for (const item of fs.readdirSync(ideas).filter((f) => f.endsWith('.json'))) {
      const idea = readJSON(inside(root, '.gameprod/ideas/' + item));
      ideaIsClosed(idea, plan);
      if (idea.decisionEvidence && !fs.existsSync(inside(root, idea.decisionEvidence))) throw Error('Missing idea decision evidence');
    }
  ...
  if (name === 'next') {
    ...
    console.log(json({ repository: plan.repository, next: tasks.filter((x) => !x.blockedBy.length).slice(0, 6), remaining: tasks.length, currentReadiness: 'not_a_release_gate' }));
    return;
  }
  if (name === 'task') {
  ...
  if (name === 'closeout') {
    ...
    const ideas = fs.existsSync(folder) ? fs.readdirSync(folder).filter((x) => x.endsWith('.json')).map((x) => readJSON(inside(root, '.gameprod/ideas/' + x))) : [];
  ...
  if (name === 'play-check') {
    if (args.length) throw Error('play-check uses the configured local inspection, no arbitrary paths');
  ...
  if (name === 'render') {
    ...
    const outputs = {
      ...
      '.gameprod/roadmap.json': json({ schemaVersion: 1, repository: plan.repository, updatedAt: plan.updatedAt, generatedFrom: PLAN_FILE, milestones: [...new Set(plan.tasks.map((t) => t.stage))].map((stage) => ({ id: stage.split('.')[0], title: stage, tasks: plan.tasks.filter((t) => t.stage === stage).map((t) => t.id) })), evidenceSemantics: plan.evidenceSemantics }),
  ...
  if (name === 'idea') {
    ...
    if (fs.existsSync(file) || plan.tasks.some((task) => task.id === id)) throw Error('ID already exists');
    withWriteLock(() => {
      if (fs.existsSync(file)) throw Error('ID already exists');
      writeJSON(file, {
      schemaVersion: 1, id, text: words.join(' '), status: 'proposed',
      createdAt: new Date().toISOString(), ownerApproval: 'not_assumed',
      requiredBeforePromotion: ['hypothesis', 'alternatives', 'bounded experiment', 'acceptance', 'tests', 'scope decision'],
  ...
  const release = name === 'experiment' ? acquireOperation(inside(root, '.gameprod/evidence/ops.lock'), { command: name, repository: plan.repository }) : null;
  try {
  const result = spawnSync(command[0], command.slice(1), {
    cwd: root, shell: false, stdio: 'inherit', timeout: 3600000
  });
