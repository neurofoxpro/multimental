...
public sealed class MultimentalBluetoothChannel : IDisposable {
 ...
 public MultimentalBluetoothChannel(string address) {
  ...
  try{handle=socket(32,1,3);if(handle==new IntPtr(-1))throw new Exception("Bluetooth socket unavailable "+WSAGetLastError());
   ...
   Address a=new Address{family=32,bluetooth=Convert.ToUInt64(address.Replace(":",""),16),service=new Guid("7e120e58-6fbd-4e4f-80e8-5685947c9dab"),port=0};
 ...
 public string Exchange(string line) {
  ...
  while(bytes.Count<16384){int n=recv(handle,one,1,0);if(n<=0)throw new Exception("RFCOMM receive failed "+WSAGetLastError());if(one[0]==10)return Encoding.UTF8.GetString(bytes.ToArray());bytes.Add(one[0]);}
