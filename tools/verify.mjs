...
for(const dir of ['scripts','tools','skills/game-production/scripts'])for(const file of fs.readdirSync(dir))if(file.endsWith('.mjs'))run(process.execPath,['--check',path.join(dir,file)]);
run(process.execPath,['--test','skills/game-production/tests/production.test.mjs','skills/game-production/tests/updater.test.mjs']);
...
const core=fs.readFileSync(path.join(root,'game/src/match_core.gd'),'utf8');if(/extends\s+(Node|Control)|get_tree\(|Time\.|HTTP|OS\./.test(core))throw Error('Domain dependency violation');console.log('ARCHITECTURE_PASS');
