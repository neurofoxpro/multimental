...
// Only a private copy is configured; never modify the user's installed editor.
export function verificationRuntime(root, executable) {
  if (process.platform !== 'win32') return executable;
  const p = readJSON(inside(root, '.gameprod/project.json'));
  context(root, p);
  if (!path.isAbsolute(executable) || !fs.existsSync(executable)) throw Error('Windows verification requires an absolute GODOT_BIN');
  const basename = path.basename(executable);
  if (!/^Godot_v4\.7\.2-stable_win64(?:_console|\.console)?\.exe$/i.test(basename)) throw Error('Unexpected local Godot binary; review the pinned adapter');
  ...
  if (fs.existsSync(target)) {
    ...
    if (owner.repository !== p.repository || owner.binaryHash !== binaryHash) throw Error('Private editor runtime belongs to different inputs');
  } else {
  ...
  for (const name of names) {
    const src = path.join(path.dirname(executable), name), dst = path.join(target, name);
  ...
  // These non-default properties are retained by the pinned 4.7.2 Windows editor.
  // Defaults and obsolete aliases are omitted by Godot on save, so do not use
  // their mere file presence as a security signal. Source hashes remain exact.
  const required = {
    ...
    'interface/editor/behavior/save_on_focus_loss': 'false'
  };
