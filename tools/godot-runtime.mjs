...
export function verificationRuntime(root, executable) {
  ...
  if (!/^Godot_v4\.7\.2-stable_win64(?:_console|\.console)?\.exe$/i.test(basename))
    throw Error('Unexpected local Godot binary; review the pinned adapter');
  ...
  for (const name of names) {
    ...
    if (fs.existsSync(dst)) {
      if (sha(fs.readFileSync(src)) !== sha(fs.readFileSync(dst)))
        throw Error('Private editor binary changed');
