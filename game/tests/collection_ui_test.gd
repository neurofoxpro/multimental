...
func run_test() -> void:
    ...
    check(ui._network_deck().ok and ui._network_deck().ids == range(15, 30), "network preparation uses the same selected deck")
    ...
    check(not ui._network_deck().ok, "network rejects missing complete selection")
    check(not ui.create_lan_room("127.0.0.1", 17854).ok and not ui.online, "invalid deck cannot host a room")
    check(not ui.join_lan_room("invalid").ok and not ui.online, "invalid deck cannot join with starter fallback")
    ui.start_match()
