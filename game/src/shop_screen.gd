...
func setup(owner_ui: Control) -> void:
    ...
    content.add_child(host.label(host.t("Тестовый магазин без реальных платежей. При первом входе выдано 500 монет — на пять паков. Крафт, награды за матчи и добровольные задания доступны из меню.", "Test shop, no real payments. First entry grants 500 coins for five packs. Crafting, match rewards and optional quests are available from the menu."), 18))
