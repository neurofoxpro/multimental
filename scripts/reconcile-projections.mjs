...
export function assertScope(env) {
  if (env.GITHUB_ACTIONS !== 'true' || env.GITHUB_EVENT_NAME !== 'push' ||
      ...
      env.GITHUB_ACTOR !== '4erk' || !SHA.test(env.GITHUB_SHA || '')) {
...
export function validateBundle(bundle, source) {
  if (bundle?.schemaVersion !== 1 || bundle.repository !== REPOSITORY ||
      bundle.branch !== BRANCH || !SHA.test(source || '') || bundle.source !== source ||
      !Array.isArray(bundle.files) || bundle.files.length > ALLOWED.size) fail('Invalid bundle identity');
  const seen = new Set();
  let size = 0;
  for (const file of bundle.files) {
    if (!ALLOWED.has(file.path) || seen.has(file.path) || typeof file.content !== 'string' ||
        file.content.includes('\0') || hash(file.content) !== file.sha256) fail('Invalid bundle file');
...
function run(args) {
  const result = spawnSync(args[0], args.slice(1), {
    shell: false, encoding: 'utf8', timeout: 180000, maxBuffer: 8000000
  });
...
function prepare() {
  ...
  const tests = fs.readdirSync('skills/game-production/tests')
  ...
  const files = run(['git', 'diff', '--name-only', '-z', source]).split('\0').filter(Boolean)
    .map((file) => {
  ...
  const bundle = validateBundle({ schemaVersion: 1, repository: REPOSITORY, branch: BRANCH, source, files }, source);
  ...
  if (process.env.GITHUB_OUTPUT) fs.appendFileSync(process.env.GITHUB_OUTPUT, 'changed=' + (files.length > 0) + '\n');
...
async function publish() {
  ...
  if (!bundle.files.length) { console.log('RECONCILE_NO_CHANGES'); return; }
  ...
  const tree = await api('/git/trees', 'POST', {
    ...
    tree: bundle.files.map((item) => ({ path: item.path, mode: '100644', type: 'blob', content: item.content }))
  });
  const commit = await api('/git/commits', 'POST', {
    message: 'chore: reconcile generated production views and formatting',
    parents: [bundle.source], tree: tree.sha
  });
  if ((await api(ref)).object.sha !== bundle.source) fail('Branch changed during prepare; safe orphan objects only');
  try {
  ...
  if (process.env.GITHUB_STEP_SUMMARY) fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,
    'Generated views committed to `' + BRANCH + '` at `' + observed + '`.\n\n' +
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  }).catch((error) => { console.error('RECONCILE_BLOCKED: ' + error.message); process.exitCode = 1; });
}
